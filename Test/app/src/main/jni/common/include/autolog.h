...
{
  public:
    CAutoLog(const s8 *szFile, const s8 *szFunction, const s32 nLine)
        : m_szCaption(szFile),
    ...
    const s8 *m_szCaption;
    const s8 *m_szFunction;
};

class CAutoLogReportTime
{
  public:
    CAutoLogReportTime(s8 *szFile, s8 *szFunction, s32 nLine)
        : m_szCaption(szFile),
          m_szFunction(szFunction)
    {
        ALOG(LOG_DEBUG, m_szCaption, "[%s@%d] enter", m_szFunction, nLine);
...
#undef USE_LOG
#define USE_LOG paomiantv::CAutoLog this_log(__FILE__, __FUNCTION__, __LINE__)
#undef USE_LOG_REPORT_TIME
#define USE_LOG_REPORT_TIME paomiantv::CAutoLogReportTime this_log(__FILE__, __FUNCTION__, __LINE__)
