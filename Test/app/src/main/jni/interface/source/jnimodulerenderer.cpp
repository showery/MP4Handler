...
namespace paomiantv {
    CLock CJNIModuleRenderer::m_SingleInstanceLock;
...
// -- noted by huangxuefeng, 2013-07-29
    ...
    CJNIModuleRenderer *CJNIModuleRenderer::CreateJniRenderer(JNIEnv *env, jobject jRenderer) {
        ...
        do {
            ...
            jfieldID jfld = env->GetFieldID(jcls, RENDERER_FIELD_NATIVE_ADDRESS_NAME,
                                            RENDERER_FIELD_NATIVE_ADDRESS_SIG);
            if (jfld == NULL) {
                LOGE("java field '%s' is not defined", RENDERER_FIELD_NATIVE_ADDRESS_NAME);
                break;
            }

            jmethodID jmtd = env->GetMethodID(jcls, RENDERER_METHOD_NEW_FRAME_NAME,
                                              RENDERER_METHOD_NEW_FRAME_SIG);
            if (jmtd == NULL) {
    ...
    CJNIModuleRenderer *CJNIModuleRenderer::GetJniRenderer(JNIEnv *env, jobject jRenderer) {
        ...
        do {
            ...
            if (nValue == 0
                ...
                if (!env->GetIntField(jRenderer, jfld) ||
                    !CJNIModuleManager::getInstance()->contains((CJNIModuleRenderer *) nValue)) {
...
//    }
    ...
    void CJNIModuleRenderer::SetFrame(unsigned char *pbyData, int nWidth, int nHeight,
        ...
        m_pRenderer->SetFrame(pbyData, nWidth, nHeight, uTimestamp, nFrameDirection,
                              bIsFrontCamera);
...
/*
    ...
    */
    ...
    void CJNIModuleRenderer::jni_Create(JNIEnv *env, jobject thiz) {
        USE_LOG;
        CJNIModuleRenderer::GetJniRenderer(env, thiz);
    }
    ...
    void CJNIModuleRenderer::jni_setTranslate(JNIEnv *env, jobject thiz, jfloat fTranslateX,
    ...
    void CJNIModuleRenderer::jni_setScale(JNIEnv *env, jobject thiz, jfloat fScaleX, jfloat fScaleY,
                                          jfloat fScaleZ) {
        LOGV("scale: x,y,z = %f,%f,%f,", fScaleX, fScaleY, fScaleZ);
        CJNIModuleRenderer *pJNIRenderer = CJNIModuleRenderer::GetJniRenderer(env, thiz);
        if (pJNIRenderer == NULL) {
            LOGE("pJNIRenderer is null!");
            return;
        }
    ...
    void CJNIModuleRenderer::jni_setFrame(JNIEnv *env, jobject thiz, jbyteArray jframeBuff,
