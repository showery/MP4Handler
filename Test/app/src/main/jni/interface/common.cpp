...
s32 getStringBytes(JNIEnv *env, jstring obj, s8 *buffer, s32 size)
{
    ...
    do
    {
        ...
        const s8 *stringPtr = (env)->GetStringUTFChars(obj, NULL);
...
s32 getStringBytesFromObjFld(JNIEnv *env, jobject obj, const s8 *fieldName, s8 *buffer, s32 size)
{
    s32 result = -1;
